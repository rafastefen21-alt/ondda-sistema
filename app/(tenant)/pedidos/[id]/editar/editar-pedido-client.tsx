...
export function EditarPedidoClient({
  ...
  const [reqDate,       setReqDate]       = useState(order.requestedDate ?? "");
  const [delivDate,     setDelivDate]     = useState(order.scheduledDeliveryDate ?? "");
  const [address,       setAddress]       = useState(order.deliveryAddress ?? "");
  const [notes,         setNotes]         = useState(order.notes ?? "");
  const [intNotes,      setIntNotes]      = useState(order.internalNotes ?? "");
    ...
    try {
      const res = await fetch(`/api/pedidos/${order.id}`, {
        ...
        body: JSON.stringify({
          ...
          requestedDate:         reqDate || null,
          scheduledDeliveryDate: delivDate || null,
      ...
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Altere itens, cliente ou dados do pedido</p>
          </div>
        </div>
      </div>
                      ...
                      {Object.entries(grouped).map(([cat, prods]) => (
                        <optgroup key={cat} label={cat}>
                          {prods.map((p) => {
                            ...
                            const displayPrice =
                              custom?.priceCaixa ?? p.priceCaixa ??
                              custom?.pricePacote ?? p.pricePacote ??
                              custom?.price ?? p.price;
                            const displayLabel = p.labelCaixa ?? p.labelPacote ?? p.unit;
                            const isSpecial = !!(custom?.priceCaixa ?? custom?.pricePacote ?? custom?.price);
                            return (
                              <option key={p.id} value={p.id}>
                                {p.name} ({displayLabel}) — {formatCurrency(displayPrice)}{isSpecial ? " ★" : ""}
                              </option>
                            );
                          })}
                        </optgroup>
