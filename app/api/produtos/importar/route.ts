...
const rowSchema = z.object({
  ...
  preco_pacote:  z.number().positive().optional(),
  rotulo_pacote: z.string().max(60).optional(),
  preco_caixa:   z.number().positive().optional(),
  rotulo_caixa:  z.string().max(60).optional(),
    ...
    try {
      await prisma.product.create({
        data: {
          ...
          pricePacote:   row.preco_pacote != null ? (row.preco_pacote as unknown as never) : null,
          labelPacote:   row.rotulo_pacote ?? null,
          priceCaixa:    row.preco_caixa  != null ? (row.preco_caixa  as unknown as never) : null,
          labelCaixa:    row.rotulo_caixa ?? null,
          ncm:           row.ncm ?? null,
          cfop:          row.cfop ?? "5102",
          active:        row.ativo,
        },
