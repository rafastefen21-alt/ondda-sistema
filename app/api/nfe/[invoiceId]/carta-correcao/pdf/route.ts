...
export async function GET(
  ...
  if (httpStatus >= 400 || !data.caminho_pdf_carta_correcao) {
    return NextResponse.json({ error: "PDF da CC-e não disponível" }, { status: 404 });
  }

  // O caminho retornado pelo Focus é relativo sem /v2 (ex: /notas_fiscais/.../cartas_correcao/1.pdf)
  const fileBase = (tenant.nfeAmbiente ?? "homologacao") === "producao"
    ? "https://api.focusnfe.com.br"
    : "https://homologacao.focusnfe.com.br";
  const pdfPath = data.caminho_pdf_carta_correcao as string;
  const pdfUrl  = pdfPath.startsWith("http") ? pdfPath : `${fileBase}${pdfPath}`;
  ...
  if (!fileRes.ok) {
    ...
    return NextResponse.json({ error: "Erro ao baixar PDF da CC-e", pdfUrl, fileStatus: fileRes.status, fileBody: body.slice(0, 500) }, { status: 502 });
  }
  ...
  return new NextResponse(fileRes.body, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="CCe-${nfe}.pdf"`,
    },
  });
}
